...
-- 메뉴별 판매수량과 금액 조회
SELECT MENU_NM, SUM(ORDER_QUENTITY) "총 판매수량", (MENU_PRICE * SUM(ORDER_QUENTITY)) ||'원' AS "총 판매금액" 
FROM MENU
...
-- 일자별 판매 수량과 금액 조회
SELECT TO_CHAR(TO_DATE(ORDER_DATE, 'YYYY-MM-DD 24HH:MI:SS') - (INTERVAL '6'HOUR), 'YYYYMMDD') AS 일자, COUNT(*) AS "총 판매수량" --,
FROM ORDER_TB 
WHERE TO_CHAR(ORDER_DATE, 'YYYY-MM-DD') = '2022-09-27'
GROUP BY ORDER_DATE;


SELECT TO_CHAR(ORDER_DATE, 'YYYY-MM-DD') 일자, (MENU_PRICE * SUM(ORDER_QUENTITY)) ||'원' AS "총 판매금액" 
FROM ORDER_TB
JOIN MENU USING(MENU_NO)
WHERE TO_CHAR(ORDER_DATE, 'YYYY-MM-DD') = '2022-09-27'
GROUP BY TO_CHAR(ORDER_DATE, 'YYYY-MM-DD'), MENU_PRICE;


SELECT TO_CHAR(TO_DATE('2022-09-27', 'YYYY-MM-DD'), 'YY"년" MM"월" DD"일"') FROM DUAL;
...
-- 당일 정산
SELECT ORDER_DATE,(MENU_PRICE * SUM(ORDER_QUENTITY)) ||'원' AS "총 판매금액" 
FROM MENU
JOIN ORDER_TB USING(MENU_NO)
GROUP BY ORDER_DATE, MENU_NM, MENU_PRICE;
