...
CREATE TABLE ORDER_TB(
	...
	SALES_QUANTITY NUMBER DEFAULT 0 NOT NULL,
	SALES_DATE DATE DEFAULT SYSDATE
);

COMMENT ON COLUMN ORDER_TB.ORDER_NO IS '주문 번호';
COMMENT ON COLUMN ORDER_TB.SALES_QUANTITY IS '판매 수량';
COMMENT ON COLUMN ORDER_TB.SALES_DATE IS '판매 날짜';
...
-- 메뉴별 판매수량과 금액 조회
SELECT MENU_NM, SUM(SALES_QUANTITY) 총 판매수량, (MENU_PRICE * SUM(SALES_QUANTITY)) ||'원' AS "총 판매금액" 
FROM MENU
